...
with open(seqFileName) as file:
    sequences = list(SeqIO.parse(file, 'fasta'))
assert len(set(len(x.seq) for x in sequences)) == 1
prefs_df = pd.read_csv(prefFileName)
print(f"Sequences read from: {seqFileName}")
print(f"There are {len(sequences)} sequences")
print(f"Each sequence is {len(sequences[0].seq) // 3} amino acids long")  # Integer division
print(f"Preferences read from: {prefFileName}")
print(f"There are preferences measured for {prefs_df.shape[0]} sites")
```
...
print(f"Sequences read from: {seqFileName}")
print(f"There are {len(sequences)} sequences")
print(f"Each sequence is {str(len(sequences[0].seq)/3)} amino acids long")
print()
for prefFileName in glob.glob("example_data/HA_prefs_*"):
  print(f"Preferences read from: {prefFileName}")
  print(f"There are preferences measured for {len(pd.read_csv(prefFileName)} sites")
```
...
for protein in proteins.keys():
  ...
  print(f"Sequences read from: {seqFileName}")
  print(f"There are {len(sequences)} sequences")
  print(f"Each sequence is {len(sequences[0].seq)/3} amino acids long")
  print(f"Preferences read from: {prefFileName}")
  print(f"There are preferences measured for {len(pd.read_csv(prefFileName))} sites")
  print()
```
